import V7.Proofs.Stage3BelowTwo.Primal
...
theorem belowPointwiseResidualIdentity : BelowPointwiseResidualIdentityStatement := by
  ...
  refine ⟨?_, ?_, ?_, ?_, ?_⟩
  · intro A B
    exact ⟨Stage3BelowTwo.forwardC n u A, (fun i => B (n - i)),
      Stage3BelowTwo.forward_map n u A B⟩
  · intro C D
    exact ⟨Stage3BelowTwo.inverseA n u C, (fun i => D (n - i)),
      Stage3BelowTwo.inverse_map n hn u dw alpha c b hcoeff C D⟩
  · intro A B C₁ D₁ C₂ D₂ hmap₁ hmap₂
    have h₁ := Stage3BelowTwo.map_determined_forward n u A B C₁ D₁ hmap₁
    have h₂ := Stage3BelowTwo.map_determined_forward n u A B C₂ D₂ hmap₂
    constructor
    · intro k hk
  ...
  · intro A B C D X hAn hX hmap
    ...
    have homega := Stage3BelowTwo.omega_block n Omega heven B D hmap.2.2
